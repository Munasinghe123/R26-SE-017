...
class ValidationService:
    ...
    @staticmethod
    def validate(ir=None, requirement_ids=None, parsed_json=None):
        requirement_ids = requirement_ids or []
        if ir is None and parsed_json is not None:
            from utils.irMapper import convert_to_ir
            ir = convert_to_ir(parsed_json)
        elif ir is not None and isinstance(ir, dict) and "class_diagram" in ir:
            from utils.irMapper import convert_to_ir
            ir = convert_to_ir(ir)
