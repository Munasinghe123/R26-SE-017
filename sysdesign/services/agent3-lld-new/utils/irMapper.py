...
def _parse_requirement_ids(raw_value: object) -> list[str]:
    if not isinstance(raw_value, list):
        return []
    return [str(req_id).strip() for req_id in raw_value if str(req_id).strip()]
...
def convert_to_ir(parsed_json: dict) -> IntermediateRepresentation:
    ...
    for raw_cls in raw_classes:
        ...
        cls = ClassIR(
            ...
            methods=methods,
            requirement_ids=_parse_requirement_ids(raw_cls.get("requirement_ids")),
        )
        classes.append(cls)
        class_index[name] = cls
    ...
    for raw_seq in raw_sequences:
        ...
        sequences.append(SequenceIR(
            ...
            messages=messages,
            requirement_ids=_parse_requirement_ids(raw_seq.get("requirement_ids")),
        ))
    ...
    for raw_entity in raw_entities:
        ...
        entity = EntityIR(
            name=name,
            attributes=attributes,
            requirement_ids=_parse_requirement_ids(raw_entity.get("requirement_ids")),
        )
