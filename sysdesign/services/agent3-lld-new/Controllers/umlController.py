...
def _build_diagram_outputs(service_result: dict) -> list[dict]:
    ...
    class_url = pngs.get("class", "")
    if class_plantuml or class_url:
        diagrams.append({
            "diagram_type": "Class",
            "plantuml_syntax": class_plantuml or "",
            "mermaid_syntax": "",
            "name": "Class Diagram",
            "cloudinary_url": class_url or "",
            "svg_content": "",
        })
    ...
    for index, seq in enumerate(seq_plantuml):
        seq_png = seq_pngs[index].get("png") if index < len(seq_pngs) else ""
        diagrams.append({
            "diagram_type": "Sequence",
            "plantuml_syntax": seq.get("plantuml", ""),
            "mermaid_syntax": "",
            "name": seq.get("name", f"Sequence {index + 1}"),
            "cloudinary_url": seq_png or "",
            "svg_content": "",
        })

    er_plantuml = plantuml.get("er", "")
    er_url = pngs.get("er", "")
    if er_plantuml or er_url:
        diagrams.append({
            "diagram_type": "ER",
            "plantuml_syntax": er_plantuml or "",
            "mermaid_syntax": "",
            "name": "ER Diagram",
            "cloudinary_url": er_url or "",
            "svg_content": "",
        })
...
class UMLController:
    ...
    @staticmethod
    async def agenerate(request: GenerateRequest):
        requirements_text, requirement_ids = _build_requirements_text(request)
        result = await UMLService.agenerate_uml(
            requirements_text,
            requirement_ids=requirement_ids,
            project_id=request.project_id,
        )
    ...
    @staticmethod
    async def agenerate_sample(sample_id: int):
        sample_path = os.path.abspath(
            os.path.join(
                os.path.dirname(__file__),
                "..",
        ...
        result = await UMLService.agenerate_uml(requirements_text)
        validation_report = result.get("validation") or {}
        validation_report["errors"] = _upper_severity(validation_report.get("errors", []))
        validation_report["iteration"] = result.get("iterations_used", 1)
