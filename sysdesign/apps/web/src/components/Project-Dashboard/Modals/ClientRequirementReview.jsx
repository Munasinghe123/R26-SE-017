...
function parseItems(clientView) {
  ...
  if (Array.isArray(clientView)) {
    clientView.forEach((item) => {
      const text =
        item.text ||
        item.description ||
  ...
  if (clientView?.items && Array.isArray(clientView.items)) {
  ...
  } else if (clientView?.specified_requirements) {
    const sr = clientView.specified_requirements;
    const all = [
      ...(sr.functional || []),
      ...(sr.non_functional || []),
      ...(sr.uncertain || []),
    ...
    all.forEach((item) => {
      const text =
        ...
        (typeof item === "string" ? item : JSON.stringify(item));
      out.push({
        id: item.id || item.requirement_id || `i-${out.length}`,
        text,
        originalText: text,
        deleted: false,
        isNew: false,
    ...
    Array.isArray(clientView?.functional) ||
    Array.isArray(clientView?.non_functional)
  ) {
    const all = [
      ...(clientView.functional || []),
      ...(clientView.non_functional || []),
      ...(clientView.uncertain || []),
    ];
    all.forEach((item) => {
      const text =
        item.description ||
...
export default function ClientRequirementReview({ onClose, projectId }) {
  ...
  const fetchReview = async () => {
    ...
    try {
      let res;
      try {
        res = await axios.get(`${AGENT1}/meetings/${projectId}/review`);
      } catch (err) {
        console.warn("Direct agent review call failed, trying proxy:", err);
        res = await axios.get(`/meetings/${projectId}/review`);
      }

      const viewData = res?.data?.client_view || res?.data?.requirements;
      if (viewData) {
        setItems(parseItems(viewData));
      }

      try {
        let qRes;
        try {
          qRes = await axios.get(`${AGENT1}/meetings/${projectId}/questions`);
        } catch {
          qRes = await axios.get(`/meetings/${projectId}/questions`);
        }
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      let res;
      try {
        res = await axios.post(`${AGENT1}/meetings/${projectId}/review`, {
          items: payload,
        });
      } catch (err) {
        console.warn("Direct review submit failed, trying proxy:", err);
        res = await axios.post(`/meetings/${projectId}/review`, {
          items: payload,
        });
      }
      ...
      if (returnedQuestions.length > 0) {
        ...
        setStage("questions");
        toast.success(
          "Changes analyzed! Please answer a few clarification questions.",
        );
      } else {
        setStage("completed");
      ...
      const msg =
        ...
        "Submit failed. Please check connection.";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmitAnswers = async () => {
    ...
    try {
      setLoading(true);
      try {
        await axios.post(`${AGENT1}/meetings/${projectId}/questions/answers`, {
          answers: payload,
        });
      } catch (err) {
        console.warn("Direct questions submit failed, trying proxy:", err);
        await axios.post(`/meetings/${projectId}/questions/answers`, {
          answers: payload,
        });
      }
      setStage("completed");
  ...
  const keptCount = items.filter(
    (i) => !i.deleted && !i.isNew && i.text === i.originalText,
  ).length;
  ...
  const reviewNeededCount = items.filter(
    (i) => !i.deleted && i.needs_review,
  ).length;

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="relative flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl border border-cyan-400/20 bg-[#080A14] shadow-[0_0_60px_rgba(34,211,238,0.2)]">
          ...
          <div className="mx-auto max-w-4xl space-y-8">
            ...
            <div className="relative p-8 rounded-3xl bg-[#0f172a]/90 border border-cyan-500/30 shadow-2xl backdrop-blur-xl space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                type="button"
                onClick={onClose}
                className="fixed right-8 top-6 z-[110] rounded-full border border-white/10 bg-black/60 p-2 text-gray-400 backdrop-blur-md transition hover:bg-white/10 hover:text-white"
                aria-label="Close requirements review"
              >
                <X size={20} />
              </button>
                  <h1
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">
                          Keep, edit, remove, or add requirements
                        </p>
                      </div>
                    </div>
                  ...
                  {items.length === 0 ? (
                    ...
                    <div className="space-y-3 pt-1">
                      {items.map((item, idx) => {
                        ...
                        const isKept =
                          !item.deleted && !item.isNew && !isEdited;
                ...
                <div className="p-6 rounded-3xl bg-[#0f172a]/95 border border-cyan-500/40 shadow-2xl backdrop-blur-xl flex flex-col md:flex-row items-center justify-between gap-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        Review your changes, then submit to analyze and
                        reconcile.
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        Please answer the questions below to finalize
                        requirement details
                      </p>
                    </div>
                  </div>
                  ...
                  {questions.map((q, idx) => (
                    ...
                    >
                          ...
                          {q.requirement_text && (
                            <div className="mt-2.5 p-3 rounded-xl bg-black/40 border border-white/10 text-xs text-cyan-200/90 leading-relaxed flex items-start gap-2">
                              <span className="shrink-0 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-cyan-500/15 text-cyan-300 border border-cyan-400/30">
                                {q.client_action === "added"
                                  ? "Added"
                                  : q.client_action === "deleted"
                                    ? "Deleted"
                                    : "Edited"}
                              </span>
                      ...
                      <div className="pl-9 space-y-3">
                        ...
                        {q.suggested_options &&
                          Array.isArray(q.suggested_options) &&
              ...
              <div className="p-12 rounded-3xl bg-[#0f172a]/90 border border-emerald-500/40 shadow-2xl backdrop-blur-xl text-center space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className="text-2xl font-bold text-white">
                    Review &amp; Clarification Complete
                  </h3>
                  <p className="text-gray-400 text-sm max-w-md mx-auto">
                    All client changes have been validated, questions answered,
                    and final requirements reconciled into the project.
                  </p>
                </div>
                <button
                  onClick={() => navigate("/project-dashboard")}
